...
export default function PlataformaAcessoForm({
    ...
    if (!isAdmin) {
        return (
            <Box sx={{ py: 4, textAlign: 'center', color: 'text.secondary' }}>
                <IconShield size={36} style={{ opacity: 0.25 }} />
                <Typography sx={{ mt: 1, fontSize: 13 }}>
                    Apenas administradores podem gerenciar acessos à plataforma.
                </Typography>
            </Box>
        );
    }
    ...
    return (
        <Card>
            <Box sx={{ px: 2.5, py: 1.8, borderBottom: '1px solid', borderColor: 'grey.100', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 1.5 }}>
                <Stack direction="row" alignItems="center" gap={1.5}>
                    <Box sx={{ width: 34, height: 34, borderRadius: 2, background: 'linear-gradient(135deg,#7c3aed,#5b21b6)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff' }}>
                        <IconShield size={17} />
                    </Box>
                    <Box>
